...
interface
uses Classes, forms,snmpsend, pingsend, asn1util, windows, ADODB;
...
begin
  ...
  begin
    AddToMemo1('Incorrect mac_wds_peer into modems with id_modem='+F_IDModem+' (mac_wds_peer='+F_mac_wds_peer+')');
...
var casse: integer; AQuery: TAdoquery;
begin
  GlobCritSect.Enter;
  AQuery := TADOQuery.Create(Application);
  AQuery.Connection := Form1.ADOConnection3;
  with  AQuery do
  begin
    Close;
...
begin
  ...
  with form1 do
     try
      ...
      MainUnit.LogError.Add('(F_IDModem='+F_IDModem+')');
