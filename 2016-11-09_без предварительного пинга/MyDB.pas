...
type
  ...
  private
    ...
    FMyStackStr: array of Ansistring;
  public
    procedure push_stack(FValue: integer; FValStr: Ansistring);
    procedure pop_stack(var FInt: integer; var FStr: Ansistring);
    constructor create;
    destructor destroy;override;
  end;
procedure AddToListFromDB(Query: TSQLQuery; List: TStrings; Pole, Table: Ansistring);
procedure AddFilesToTable(TblFilesName:AnsiString;Fields, Values: array of AnsiString);
procedure AddToTable(Table: AnsiString; Fields, Values: array of AnsiString);
procedure AddToStats_operTable(Operation: AnsiString);
procedure UpdateToTable(Table, KeyField, KeyValue: AnsiString; Fields, Values: array of AnsiString);
procedure UpdateToFilesTable(TblFilesName, KeyField, KeyValue: AnsiString; Fields, Values: array of AnsiString);
function DeleteFromTable(Table, KeyField, ValueKeyField: AnsiString; ShowConf: boolean): boolean;
function DeleteFromTableWhere(Table, FWhere: AnsiString): boolean;
function CountRecordInMySQLTable(Query: TSQLQuery; Pole, Znach, Base: AnsiString): longint;
function FindIndex(Value: Integer; ArrInt: array of integer): Integer;
function StrToHex(Value: AnsiString): AnsiString;
function Crpt(value: AnsiString): AnsiString;
function ifNullFile(id,KeyName, TableFilesName: AnsiString): boolean;
function ifNullFileAkt(id,KeyName, TableFilesName: AnsiString): boolean;
function ifNullFileMeropr(id,KeyName, TableFilesName: AnsiString): boolean;
function isNullField(Table,Field,id,Value_id: AnsiString): boolean;
function MaxValue(Table, Field: AnsiString): Integer;
function Null_Query(var Query: TSQLQuery; TableName, Uslovie: AnsiString): boolean;
...
function IsOLEObjectInstalled(Name: AnsiString): boolean;
...
function GetAdapterInfo(Lana: Char): AnsiString;
var
...
var
  i: integer;
  query, query_1, Table: AnsiString;
begin
  Table :=TblFilesName;
  query := 'Update ' + Table + ' set ';
  query_1 := query;
...
function WhichLanguage:AnsiString;
var
  ID: LangID;
...
procedure TMyStack.pop_stack(var FInt: integer; var FStr: AnsiString);
begin
...
procedure TMyStack.push_stack(FValue: integer; FValStr: AnsiString);
begin
