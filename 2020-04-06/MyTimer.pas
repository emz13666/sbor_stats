...
begin
  ...
  repeat
    ...
    if IfOfflineMore5min {ArrayIdModems5MinNoPing[StrToInt(F_IDModem)] = 1} then
    begin
...
try
  AQuery.Close;
  AQuery.SQL.Text := 'Select is_ap_repeater from modems where id_modem =' + F_IDModem;
  try
    AQuery.Open;
    AQuery.First;
    casse := AQuery.FieldByName('is_ap_repeater').AsInteger;
    AQuery.Close;
      case casse of
