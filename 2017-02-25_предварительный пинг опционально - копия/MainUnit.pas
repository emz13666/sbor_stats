...
uses
  ...
  ADODB, SyncThread, MoveToStatss_oldThread, Menus, ThreadTimerWifiOff, Spin;// DelphiCryptlib, cryptlib;
...
begin
  ...
  try
    Modems.SQL.Text := 'SELECT m.id_modem, m.is_access_point, m.firmware, e.name, e.ip_address, e.equipment_type,'+
     ' e.useInMonitoring  FROM modems m, equipment e WHERE e.equipment_type<=3 and e.useInMonitoring=1 and '+
