...
type TSSHobj=class
  ...
  public
    ...
    procedure Unlock;
    constructor Create(FIP,FLogin,FPass,Fhellostr: AnsiString);
    destructor Destroy; override;
    function Execute : boolean;
end;

function GetWord(Str, Smb: AnsiString; WordNmbr: Byte): AnsiString;

// Ќайти первый индекс строки списка, в которую входит подстрока. ¬озвращает номер строки списка или -1, если не найден
function FindLineSubstringInList(substring:AnsiString;var List:TStrings ):integer;


implementation

function FindLineSubstringInList( substring:AnsiString;var List:TStrings ):integer; // Ќайти первый индекс строки списка, в которую входит подстрока. ¬озвращает номер строки списка или -1, если не найден
begin
...
constructor TSSHobj.Create(FIP,FLogin,FPass,Fhellostr: AnsiString);
begin
     inherited Create;
     hellostr := Fhellostr;
...
function TSSHobj.SSH_Client(Server, Userid, Pass: Ansistring): TCryptSession;
begin
