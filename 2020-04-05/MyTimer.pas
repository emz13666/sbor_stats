...
type
  ...
  protected
    ...
    procedure AddToMemo1(f_str: Ansistring);
    procedure SaveToLocalDB;
    procedure WriteToForm_AP;
    procedure SaveToLocalDB_AP;
    procedure SaveFirmwareToMySQL;
    procedure ReadAndUpdateFirmwareVer;
    procedure ReadIsAP_RepeaterFromMySQL;
...
procedure TMyTimerThread.AddToMemo1(f_str: Ansistring);
begin
  GlobCritSect.Enter;
  Form1.Memo1.Lines.Add(FormatDateTime('dd.mm.yyy hh:nn:ss - ', now) +f_str);
  GlobCritSect.Leave;
end;
...
begin
  ...
  repeat
    ...
    begin
     ...
     if (GetTickCount - timer_5_min) > 300000 then
     begin
       if f_chk_ver  then
       begin
         ReadAndUpdateFirmwareVer;
       end;
       ReadIsAP_RepeaterFromMySQL;
       timer_5_min := GetTickCount;
     end;
  until Terminated;
end;

procedure TMyTimerThread.ReadAndUpdateFirmwareVer;
     ...
     try
       ...
       if snmp.SendRequest then
         begin
           ...
           if s3<>f_firmware then
           begin
             f_firmware := s3;
             SaveFirmwareToMySQL;
             if f_firmware = '5.5' then f_new := false else f_new := true;
           end;
         end;
       snmp.Free;
     except
      ...
      begin
       snmp.Free;
       GlobCritSect.Enter;
       SaveLogToFile(LogFileName,'Ошибка в ReadAndUpdateFirmwareVer. Modem:'+f_nameModem+' ('+E.ClassName+': '+E.Message+')');
       GlobCritSect.Leave;
      end;
     end;
end;
...
procedure TMyTimerThread.ReadIsAP_RepeaterFromMySQL;
var casse: integer; AQuery: TAdoquery;     AConn: TADOConnection;
begin
  GlobCritSect.Enter;
  ...
  begin
    Close;
    SQL.Text := 'Select is_ap_repeater from modems where id_modem =' + F_IDModem;
    try
      Open;
      First;
      casse := FieldByName('is_ap_repeater').AsInteger;
      Close;
      free;
      AConn.Close;
      AConn.Free;
      case casse of
         0: f_is_ap_repeater := false;
         1: f_is_ap_repeater := true;
      end;
      GlobCritSect.Leave;
    except
      on E: Exception do
      ...
      AConn.Close;
      AConn.Free;
      SaveLogToFile(LogFileName,'Ошибка при проверке ReadIsAP_RepeaterFromMySQL. Modem:'+f_nameModem+' ('+E.ClassName+': '+E.Message+')');
      GlobCritSect.Leave;
      end;
    end;
  end;
...
procedure TMyTimerThread.SaveFirmwareToMySQL;
var AQuery: TADOQuery; AConn: TADOConnection;
begin
  GlobCritSect.Enter;
  try
    AQuery := TADOQuery.Create(Application);
  ...
  AQuery.Connection := AConn;
    ...
    begin
      Close;
      SQL.Text := 'Update modems set firmware=' + QuotedStr(f_firmware)+' where id_modem='+F_IDModem;
      ExecSQL;
      Close;
    end;
    AQuery.Free;
    AConn.Close; AConn.Free;
    GlobCritSect.Leave;
  except
    on E:Exception do
    begin
    ...
    AQuery.Free;
    AConn.Close; AConn.Free;
    SaveLogToFile(LogFileName,'Ошибка в SaveFirmwareToMySQL. Modem:'+f_nameModem+' ('+E.ClassName+': '+E.Message+')');
    GlobCritSect.Leave;
    end;
  end;
end;
